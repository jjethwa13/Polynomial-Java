...
class NewtonFractal {
    ...
    public void printRoots() {
    	...
    	for (int i = 0; i < this.roots.size(); i++) {
    		System.out.println("z" + i + " = " + this.roots.get(i));
    	}
    }
    ...
    public int findRoot(Complex root) {
    	...
    	for (int i = 0; i < this.roots.size(); i++) {
    		if (Math.abs(this.roots.get(i).add(root).abs()) < Newton.TOL) {
    			index = i;
    		}
    	}
    	if (index == -1) {
    		this.roots.add(root.minus());
    	}
    ...
    public Complex pixelToComplex(int i, int j) {
    	...
    	z = this.origin;
    	z = z.add(new Complex(dz*i, 0));
    	z = z.add(new Complex(0, -dz*j));
    ...
    public void createFractal(boolean colorIterations) {
    	...
    	for (int k = 0; k < NUMPIXELS; k++) {
    		for (int j = 0; j < NUMPIXELS; j++) {
    			z = this.pixelToComplex(j, k);
    			
    			this.iterator.iterate(z);
    			if (this.iterator.getError() == 0) {
    				indexcolor = this.findRoot(this.iterator.getRoot());
    				if (indexcolor == -1) {
    					continue;
    				}
    				else {
    					this.colorPixel(j, k, indexcolor, this.iterator.getNumIterations());
    				}
    			}
    		}
    	}
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff = new Complex[] { new Complex(-1, 0), new Complex(), 
